<script setup lang="ts">
 ...
 let alert = computed<boolean>(()=>{return alertStore.getAlert})
 let message = computed<string>(()=>{return alertStore.getMessage})

 function closeModal(){alertStore.changeAlert("")}
</script>
...
<template>
  ...
  >
    <div
        ...
        :class="getDarkModeStatus() ? 'bg-innerDark/60':'bg-indigo-500/25'"
    >
      <section
          v-motion="StaggerEffect(575,-100,0)"
          class="border-2 rounded-md lg:w-1/2 w-[18rem]  flex flex-col space-y-4 py-6 items-center"
          :class="getDarkModeStatus() ? 'bg-teal-950 border-indigo-500 border-dashed text-indigo-500'
